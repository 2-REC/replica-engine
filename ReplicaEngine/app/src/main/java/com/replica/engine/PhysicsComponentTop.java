 ...
 */
public class PhysicsComponentTop extends GameComponent {
    ...
    public void reset() {
        ...
        setStaticFrictionCoeffecient(DEFAULT_STATIC_FRICTION_COEFFICIENT);
        setDynamicFrictionCoeffecient(DEFAULT_DYNAMIC_FRICTION_COEFFICIENT);
    }
...
//            final Vector2 gravityVector = gravity.getGravity();
            final float gravityVector = gravity.getGravity();

            if (Math.abs(newVelocity.x) > 0.0f) {
                ...
                final float maxFriction = Math.abs(gravityVector) * getMass() * frictionCoefficient;
            ...
            if (Math.abs(newVelocity.y) > 0.0f) {
                // if we were moving last frame, we'll use dynamic friction. Else
                // static.
                ...
                if (maxFriction > Math.abs(newVelocity.y)) {
                    newVelocity.y = (0.0f);
                } else {
                    newVelocity.y = (newVelocity.y - (maxFriction * Utils.sign(newVelocity.y)));
                }
            }
        }
