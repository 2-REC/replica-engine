...
public class NPCComponent extends GameComponent {
    ...
    public void reset() {
        ...
        mLastHitTileX = 0;
        mLastHitTileY = 0;
        mDialogEvent = GameFlowEvent.EVENT_SHOW_DIALOG_CHARACTER;
        mDialogIndex = 0;
        ...
        case HotSpotSystem.HotSpotType.TALK:
        	if (mHitReactComponent != null) {
            	if (parentObject.lastReceivedHitType != HitType.COLLECT) {
            		mHitReactComponent.setGameEventOnTakeHit(
            		...
            		resumeMovement(parentObject);	                        		
//            		mHitReactComponent.setGameEventOnTakeHit(HitType.INVALID, 0, 0);
            		mHitReactComponent.setGameEventOnTakeHit(HitType.COLLECT, -1, 0);
...
// !!!! TODO: check that OK !!!!
        default:
            if (hotSpot >= HotSpotSystem.HotSpotType.NPC_SELECT_DIALOG) {
    ...
    private void selectDialog(int hitSpot) {
		mDialogEvent = GameFlowEvent.EVENT_SHOW_DIALOG_CHARACTER;
    	mDialogIndex = hitSpot - HotSpotSystem.HotSpotType.NPC_SELECT_DIALOG;
    }
