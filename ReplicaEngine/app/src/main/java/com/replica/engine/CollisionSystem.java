...
*/
//////// size - m
        // This enables to have collision tiles of arbitrary sizes.
        // In the raw file sizes are given regarding the tile (eg: values between 0.0f-1.0f).
        // When setting "width" & "height" here, the values are scaled to be relative to the new tiles sizes.
        if (tileWidth != mTileWidth || tileHeight != mTileHeight) {
            final int oldWidth = mTileWidth;
            final int oldHeight = mTileHeight;
            final int newWidth = tileWidth;
            final int newHeight = tileHeight;

            for (int i = 0; i < mCollisionTiles.length; i++) {
                final FixedSizeArray<LineSegment> segments = mCollisionTiles[i].segments;
                final int count = segments.getCount();
                final Object[] segmentArray = segments.getArray();

                for (int j = 0; j < count; j++) {
                    final LineSegment segment = (LineSegment) segmentArray[j];
                    segment.mStartPoint.x /= oldWidth;
                    segment.mStartPoint.y /= oldHeight;
                    segment.mEndPoint.x /= oldWidth;
                    segment.mEndPoint.y /= oldHeight;
                    segment.mNormal.multiply(10.0f); // make sure it doesn't become too small
                    segment.mNormal.x /= oldWidth;
                    segment.mNormal.y /= oldHeight;

                    segment.mStartPoint.x *= newWidth;
                    segment.mStartPoint.y *= newHeight;
                    segment.mEndPoint.x *= newWidth;
                    segment.mEndPoint.y *= newHeight;
                    segment.mNormal.x *= newWidth;
                    segment.mNormal.y *= newHeight;
                    segment.mNormal.normalize();
                }
            }
            mTileWidth = newWidth;
    ...
    public boolean loadCollisionTiles(InputStream stream) {
        ...
        try {
            ...
            if (signature == 52) {
                ...
                mCollisionTiles = new CollisionTile[tileCount];
