...
public class TiledVertexGrid extends BaseObject {
    ...
    protected Grid mTileMap;
    protected TiledWorld mWorld;
    ...
    protected float mWorldPixelWidth;
    protected float mWorldPixelHeight;
    
    protected float mWidthSizeFactor = 1.0f;
    protected float mHeightSizeFactor = 1.0f;

    protected int mTilesPerRow;
    protected int mTilesPerColumn;
    
    protected Boolean mGenerated;
    
    public TiledVertexGrid(Texture texture, int width, int height, int tileWidth, int tileHeight,
            float widthSizeFactor, float heightSizeFactor ) {
        super();
        mTileWidth = tileWidth;
        mTileHeight = tileHeight;
        mWidth = width;
        mHeight = height;
        mWidthSizeFactor = widthSizeFactor;
        mHeightSizeFactor = heightSizeFactor;
        mTexture = texture;
        mGenerated = false;
    }
    ...
    protected Grid generateGrid(int width, int height, int startTileX, int startTileY) {
        ...
        final float widthSizeFactor = mWidthSizeFactor;
        final float heightSizeFactor = mHeightSizeFactor;
        ...
        if (!entirelyEmpty) {
            ...
            for (int tileY = 0; tileY < tilesDown; tileY++) {
                for (int tileX = 0; tileX < tilesAcross; tileX++) {
                    ...
                    final float[] p0 = { offsetX * widthSizeFactor, offsetY * heightSizeFactor, 0.0f };
                    final float[] p1 = { (offsetX + tileWidth) * widthSizeFactor, offsetY * heightSizeFactor, 0.0f };
                    final float[] p2 = { offsetX * widthSizeFactor, (offsetY + tileHeight) * heightSizeFactor, 0.0f };
                    final float[] p3 = { (offsetX + tileWidth) * widthSizeFactor, (offsetY + tileHeight) * heightSizeFactor, 0.0f };
        ...
        mTileWidth = (int)(tileWidth * widthSizeFactor);
        mTileHeight = (int)(tileHeight * heightSizeFactor);
        mWorldPixelWidth *= widthSizeFactor;
        mWorldPixelHeight *= heightSizeFactor;
