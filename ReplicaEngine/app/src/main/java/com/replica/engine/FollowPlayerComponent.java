...
/**
 * Manages the position of an object based on the player's position.
 */
public class FollowPlayerComponent extends GameComponent {
...
// => should change priority? (or add target's velocity to position?
    ...
    public void update(float timeDelta, BaseObject parent) {
        ...
        if (manager != null) {
            final GameObject target = manager.getPlayer();
            if (target != null) {
                if (!mAttached) {
...
// !!!! ???? TODO: '+' or '-' offsets (y?) ? ???? !!!!
                    final float offsetX = (mFollowOffsetXAbsolute) ? mFollowOffsetX : (mFollowOffsetX * target.facingDirection.x);
                    final float offsetY = (mFollowOffsetYAbsolute) ? mFollowOffsetY : (mFollowOffsetY * target.facingDirection.y);
                    mTargetPosition.set(target.getCenteredPositionX() + offsetX, target.getCenteredPositionY() + offsetY);
...
// !!!! TODO: handle offset from parent object !!!!
                    final GameObject parentObject = (GameObject)parent;
//                    parentObject.getPosition().set(target.getCenteredPositionX() + offsetX, target.getCenteredPositionY() + offsetY);
                    parentObject.getPosition().x = target.getCenteredPositionX() + offsetX - (parentObject.width / 2.0f);
                    parentObject.getPosition().y = target.getCenteredPositionY() + offsetY - (parentObject.height / 2.0f);
                }
            }
        }
    }
